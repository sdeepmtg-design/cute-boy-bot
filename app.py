...
class CuteBoyBot:
    ...
    def should_send_voice(self, message):
        ...
        # РАСШИРЕННЫЙ СПИСОК ТРИГГЕРОВ - отправляем голосовое в 70% случаев
        voice_triggers = [
            # Приветствия
            'привет', 'hello', 'hi', 'хай', 'ку', 'здравствуй', 'добрый',
            # Вопросы
            'как дела', 'как ты', 'что делаешь', 'как жизнь', 'чего', 
            'как настроение', 'чем занят', 'как день',
            # Ночные
            'спокойной ночи', 'доброй ночи', 'спок', 'ночи', 'сладких снов',
            # Положительные эмоции
            'хорошо', 'отлично', 'прекрасно', 'супер', 'замечательно', 'класс',
            # Скука/тоска
            'скучаю', 'соскучилась', 'miss you', 'грустно', 'одиноко',
            # Любовь/симпатия
            'люблю', 'обожаю', 'симпатия', 'нравишься', 'любимый',
            # Тестовые/прямые запросы
            'голос', 'voice', 'говори', 'озвучь', 'расскажи', 'позвони',
            'а будет голосовое', 'голосовое', 'можно голосовое',
            # Короткие сообщения (1-2 слова)
            'да', 'нет', 'ок', 'угу', 'ага', 'ладно', 'поняла'
        ]
        
        # Проверяем триггеры
        has_trigger = any(trigger in message_lower for trigger in voice_triggers)
        
        # Также отправляем голосовое для очень коротких сообщений
        is_short = len(message.split()) <= 2
        
        # Высокий шанс для тестирования
        should_send = (has_trigger or is_short) and random.random() < 0.7
        
        logger.info(f"🎤 Voice check: '{message}' -> trigger:{has_trigger} short:{is_short} -> send:{should_send}")
        return should_send
