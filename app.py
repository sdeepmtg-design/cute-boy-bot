...
# Настройки подписки
TRIAL_MESSAGES = 5
SUBSCRIPTION_PRICE_WEEK = 29900  # в копейках
SUBSCRIPTION_PRICE_MONTH = 99900  # в копейках

# База данных в памяти (в продакшене заменить на Redis/БД)
users_db = {}
...
def create_yookassa_payment(user_id, plan_type, chat_id):
    """Создание платежа в ЮКассе"""
    try:
        if not YOOKASSA_SHOP_ID or not YOOKASSA_SECRET_KEY:
            logging.error("Yookassa credentials not set")
            return None

        if plan_type == "week":
            amount = SUBSCRIPTION_PRICE_WEEK
            description = "Подписка на неделю"
            days = 7
        else:  # month
            amount = SUBSCRIPTION_PRICE_MONTH
        ...
        payload = {
            ...
            "confirmation": {
                "type": "redirect",
                "return_url": f"{RENDER_URL}/payment_success"
            },
            "capture": True,
        ...
        if response.status_code == 200:
            ...
            pending_payments[payment_data['id']] = {
                'user_id': user_id,
                'plan_type': plan_type,
                'chat_id': chat_id,
                'days': days
            ...
            logging.info(f"💰 Payment created: {payment_data['id']}")
...
def activate_subscription(user_id, plan_type, days):
    """Активация подписки"""
    try:
        users_db[user_id] = {
            'messages_used': 0,
        ...
        logging.info(f"✅ Subscription activated for {user_id}: {plan_type} for {days} days")
        return True
    except Exception as e:
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('week_') or query.data.startswith('month_'):
        ...
        payment_url = create_yookassa_payment(target_user_id, plan_type, chat_id)
        
        if payment_url:
            await query.edit_message_text(
                text=f"💫 Перейди по ссылке для оплаты подписки {'на неделю' if plan_type == 'week' else 'на месяц'}:\n\n{payment_url}\n\nПосле оплаты подписка активируется автоматически! ✅",
                reply_markup=None,
                parse_mode='Markdown'
            )
        else:
            await query.edit_message_text(
                text="❌ Ошибка при создании платежа. Попробуй еще раз.",
                reply_markup=None
            )
...
def payment_webhook():
    ...
    try:
        data = request.json
        logging.info(f"💰 Payment webhook received: {data['event']}")
        
        if data['event'] == 'payment.succeeded':
            payment_id = data['object']['id']
            payment_data = pending_payments.get(payment_id)
            
            if payment_data and telegram_app:
                ...
                if success:
                    telegram_app.bot.send_message(
                        ...
                        text=f"✅ Подписка активирована! Спасибо за оплату! 🎉\n\nТеперь можно общаться без ограничений! 💫"
                    )
...
def payment_success():
    ...
    <html>
        <body style="font-family: Arial, sans-serif; text-align: center; padding: 50px;">
            <h1 style="color: #4CAF50;">✅ Оплата прошла успешно!</h1>
            <p>Подписка активируется автоматически. Вернитесь в Telegram для продолжения общения.</p>
        </body>
    </html>
    """
...
def home():
    return jsonify({
        "status": "healthy",
        "bot": "Virtual Boy 🤗",
        "features": ["deepseek", "subscriptions", "yookassa_payments", "conversation_memory"]
    })
...
def start_bot():
    ...
    if telegram_app:
        # Для Render используем вебхуки, поэтому не запускаем polling
