...
class VirtualBoyBot:
	...
	СЕГОДНЯШНЯЯ ДАТА: {today.strftime("%d %B %Y")} ({today.strftime("%A")}, {today.strftime("%B")})
	...
	- УПОМИНАЙ ТЕКУЩУЮ ДАТУ, ВРЕМЯ ГОДА, ПРАЗДНИКИ когда это уместно
	- Знаешь какой сегодня день недели, месяц, время года
        
        ПРАЗДНИКИ И ВРЕМЕНА ГОДА:
        - Зима: Декабрь, Январь, Февраль (Новый год, Рождество, 14 февраля)
        - Весна: Март, Апрель, Май (8 Марта, Пасха, 1 Мая)
        - Лето: Июнь, Июль, Август (1 Июня, 12 Июня, 22 Июня)
        - Осень: Сентябрь, Октябрь, Ноябрь (1 Сентября, 4 Ноября)
