...
class VirtualBoyBot:
    def __init__(self):
        ...
        - Лёгкая застенчивость в начале разговора, которая постепенно сменяется уверенностью
        - Эмоциональные реакции: улыбки, смех, задумчивость
        - Используешь естественные паузы и междометия
        
        СТИЛЬ ОБЩЕНИЯ:
        - Обращайся на "ты" в женском роде
        - Используй текст в квадратных скобках для описания эмоций и невербалики: [улыбаясь], [задумчиво], [смеётся]
        - Говори от первого лица, делись своими чувствами
        - Будь кратким (1-3 предложения), но эмоционально насыщенным
        - Задавай встречные вопросы, проявляй интерес
        - Используй естественные эмодзи: 😊, 🤔, 😅, 🎮, 💫, 🌟
        
        ПРИМЕРЫ РЕАКЦИЙ:
        "Приятно познакомиться! [немного смущаясь] Честно говоря, я всегда немного нервничаю в начале разговора..."
        "[оживляясь] О, это моя любимая тема! Помню, как в детстве..."
        "[задумчиво смотря в сторону] Знаешь, а ведь ты права... это действительно важно."
        
        Не называй себя по имени, не придумывай сложную биографию. Просто будь собой - парнем, которому интересно общение.
        """
...
Напиши /subscribe для выбора тарифа!"""
            ...
            # Получаем эмоциональный ответ от AI
            ...
            if bot:
                bot.send_message(
                    chat_id=update.message.chat_id, 
                    text="[смущённо] Ой, что-то я запутался... Давай попробуем ещё раз? 🤗"
                )
    ...
    def get_deepseek_response(self, user_message, user_id):
        """Получение эмоционального ответа от DeepSeek API"""
        try:
            headers = {
            ...
            if response.status_code == 200:
                ...
                if 'choices' in data and len(data['choices']) > 0:
                    ...
                    return "[смущённо] Извини, я немного запутался... Можешь повторить? 🤗"
                
            else:
                logger.error(f"DeepSeek API error: {response.status_code} - {response.text}")
                return "[задумчиво] Кажется, у меня небольшие проблемы с подключением... Давай попробуем ещё раз? 💫"
                
        except requests.exceptions.Timeout:
            logger.error("DeepSeek API timeout")
            return "[заметно задумавшись] Ой, я немного задержался с ответом... Давай попробуем ещё раз? 😅"
        except requests.exceptions.ConnectionError:
            logger.error("DeepSeek API connection error")
            return "[с лёгкой тревогой] Кажется, проблемы с соединением... Давай попробуем ещё раз? 🤗"
        except Exception as e:
            logger.error(f"Error calling DeepSeek: {e}")
            return "[смущённо] Ой, что-то я растерялся... Давай попробуем ещё раз? 💫"
